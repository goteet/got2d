...
void ComponentContainer::OnPositionChanged(const cxx::point2d<float>& p)
{
	for (ComponentRecord& c : mComponents)
	{
		c.ComponentPtr->OnPositionChanged(p);
	}
}
void ComponentContainer::OnPivotChanging(const cxx::float2& p)
{
	for (ComponentRecord& c : mComponents)
	{
		c.ComponentPtr->OnPivotChanging(p);
	}
}

void ComponentContainer::OnPivotChanged(const cxx::float2& p)
{
	for (ComponentRecord& c : mComponents)
	{
		c.ComponentPtr->OnPivotChanged(p);
	}
}
